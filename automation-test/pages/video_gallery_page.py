"""
Video Gallery Page Object - Video
"""
import time
import os
...
class VideoGalleryPage(BasePage):
    """Video Gallery management page object"""

    # URLs
    GALLERY_URL = f"{Config.BASE_URL}/admin/video-galleries"
    CREATE_URL = f"{Config.BASE_URL}/admin/video-galleries/create"

    def get_edit_url(self, gallery_id):
        """Get edit URL"""
        return f"{Config.BASE_URL}/admin/video-galleries/{gallery_id}/edit"

    # Locators - List View
    CREATE_LINK = (By.XPATH, "//a[contains(@href, '/video-galleries/create')]")
    ...
    # Form Fields (Based on VideoGalleryResource.php)
    ...
    VIDEO_INPUT = (By.CSS_SELECTOR, "input[type='file']")
    ...
    def navigate(self):
        ...
        print(f"Opening video gallery page: {self.GALLERY_URL}")
    ...
    def is_on_gallery_page(self):
        ...
        return '/admin/video-galleries' in self.get_current_url()

    def click_create_button(self):
        """Click create button"""
        print("Opening create video gallery page...")
    ...
    def fill_gallery_form(self, title, category=None, description=None, video_path=None):
        """
        Fill video gallery form

        Args:
            title (str): Video title (required)
            category (str): Kegiatan, Infrastruktur, Alam, Budaya
            description (str): Description
            video_path (str): Path to video file (required)

        Returns:
            bool: Success status
        """
        print(f"Filling video gallery form: {title}")
        ...
        try:
            ...
            # 4. Upload video
            if video_path:
                file_input = self.find_element(*self.VIDEO_INPUT)
                file_input.send_keys(video_path)
                time.sleep(5)  # Video uploads take longer
    ...
    def find_video_in_table(self, title):
        """Find video by title"""
    ...
    def click_edit_for_video(self, title):
        """Click edit for video"""
        row = self.find_video_in_table(title)
        if row:
            try:
